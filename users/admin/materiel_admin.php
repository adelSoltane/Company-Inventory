<?php include 'header.php'; ?>

<?php include 'server_materiel.php'; 

  // fetch the record to update
 if (isset($_GET['edit'])) {
    $id = $_GET['edit'];

    $edit_state = true;
    $rec = mysqli_query($conn, "SELECT * FROM materiel WHERE id_materiel=$id");
    $record = mysqli_fetch_array($rec);
    $date = $record['date_acquis'];
    $model = $record['model'];
    $id = $record['id_materiel']; 
    $marque = $record['marque']; 
    $garantie = $record['garantie']; 
    
    $libelle = $record['libelle']; 
    $code = $record['code'];
    $prix = $record['prix'];
    $fournisseur = $record['fournisseur']; 
 }

?>

<h1>Matériel</h1>
<?php echo $success; echo $error; echo $error2; echo $empty; echo $del;?>
    <table>
      <thead>
        <tr>
        <!-- Button trigger modal -->
<button type="button" class="btn btn-primary d-print-none" data-toggle="modal" data-target="#ModalAdd" style="margin-top: 20px; margin-left: 120px;">
  Ajouter un matériel
</button>

<a hidden-print href="" id="print" onclick="javascript:window.print()" class="btn btn-primary d-print-none" style="display: inline-block; margin-top: 20px; margin-left: 840px;">
  Imprimer
</a>

<form action="materiel_admin.php" method="GET">
  <input type="text" style="position: relative; width: 80%; margin-left: 120px; position: relative; margin-top: 20px;" class="form-control d-print-none" placeholder="Recherche" name="motCle" value="<?php echo ($mc) ?>"/>
</form>

<!-- Modal Update-->
<div class="modal fade" id="ModalAdd" tabindex="-1" role="dialog" aria-labelledby="ModalUpdateLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalAddLabel">Mettre à jour</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form method="POST" action="materiel_admin.php">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <div class="input-group">
        <label>Libellé</label>
        <input type="text" name="libelle" value="<?php echo $libelle; ?>">
      </div>
      <label>Date</label>
      <div class="input-group">
        
        <input type="date" name="date" value="<?php echo $date; ?>">
      </div>
      <div class="input-group">
        <label>Model</label>
        <input type="text" name="model" value="<?php echo $model; ?>">
      </div>
      <div class="input-group">
        <label>Marque</label>
        <input type="text" name="marque" value="<?php echo $marque; ?>">
      </div>
      <div class="input-group">
        <label>Garantie</label>
        <input type="date" name="garantie" value="<?php echo $garantie; ?>">
      </div>
      <div class="input-group">
        <label>N° Inventaire</label>
        <input type="number" name="code" value="<?php echo $code; ?>">
      </div>
      <div class="input-group">
        <label>Prix du matériel ( Dinard Algérien )</label>
        <input type="number" name="prix" value="<?php echo $prix; ?>">
      </div>
      <div class="input-group">
        <label>Fournisseur</label>
        <input list="browsers" class="input-group" name="fournisseur" value="<?php echo $fournisseur ?>" >
        <datalist id="browsers">
          
        
        <?php while ($row = mysqli_fetch_array($recordd)) {

          echo '<option value="'.$row['LibelleFournisseurs'].'" >'.$row['IdFournisseur'].' </option>';
            }
            ?>
            
            </datalist>
      </div>
      
      <div class="input-group">
      <?php if ($edit_state == false): ?>
        <button type="submit" name="save" class="btn">Sauvegarder</button>
      <?php else: ?>
        <button type="submit" name="update" class="btn">Mettre à jour</button>
      <?php endif ?>
      </div>
    </form>
      </div>
    
    </div>
  </div>
</div>
          <th>Libellé</th>
          <th>Date</th>
          <th>Model</th>
          <th>Marque</th>
          <th>Garantie</th>
          <th>Inventaire</th>
          <th>Prix</th>
          <th>Fournisseur</th>
          
          <th colspan="1" class="d-print-none">Action</th>

        </tr>
      </thead>
      <tbody>
        <?php while ($row = mysqli_fetch_array($results)) { ?>
          <tr>
          <td><?php echo $row['libelle']; ?></td>
          <td><?php echo $row['date_acquis']; ?></td>
          <td><?php echo $row['model']; ?></td>
          <td><?php echo $row['marque']; ?></td>
          <td><?php echo $row['garantie']; ?></td>
          <td><?php echo $row['code']; ?></td>
          <td><?php echo $row['prix']; ?></td>
          <td><?php echo $row['fournisseur']; ?></td>
          
          
          <td align="right">
            <a hidden-print class="btn btn-primary d-print-none" href="materiel_admin.php?edit=<?php echo $row['id_materiel']; ?>">Modifier</a>
          
            <a hidden-print class="btn btn-danger d-print-none" href="materiel_admin.php?del=<?php echo $row['id_materiel']; ?>">Effacer</a>
          </td>
        </tr>
        

        <?php } ?>
        
      </tbody>
    </table>

    <?php for($i=1;$i<=$nbpage;$i++){
      echo "<a style=\"position: relative;
    left: 550px;\" class=\"btn btn-secondary d-print-none\" href=\"materiel_admin.php?p=$i\">$i </a>";
      } 
    
    ?>

    <form method="POST" action="server_materiel.php">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <div class="input-group">
        <label>Libellé</label>
        <input type="text" name="libelle" value="<?php echo $libelle; ?>">
      </div>
      <label>Date</label>
      <div class="input-group">
        
        <input type="date" name="date" value="<?php echo $date; ?>">
      </div>
      <div class="input-group">
        <label>Model</label>
        <input type="text" name="model" value="<?php echo $model; ?>">
      </div>
      <div class="input-group">
        <label>Marque</label>
        <input type="text" name="marque" value="<?php echo $marque; ?>">
      </div>
      <div class="input-group">
        <label>Garantie</label>
        <input type="date" name="garantie" value="<?php echo $garantie; ?>">
      </div>
      <div class="input-group">
        <label>N° Inventaire</label>
        <input type="text" name="code" value="<?php echo $code; ?>">
      </div>
      <div class="input-group">
        <label>Prix du matériel ( Dinard Algérien )</label>
        <input type="text" name="prix" value="<?php echo $prix; ?>">
      </div>
      <div class="input-group">
        <label>Fournisseur</label>
        <input list="browsers" class="input-group" name="fournisseur" value="<?php echo $fournisseur ?>" >
        <datalist id="browsers">
          
        
        <?php while ($row = mysqli_fetch_array($recordd)) {

          echo '<option value="'.$row['LibelleFournisseurs'].'" >'.$row['IdFournisseur'].' </option>';
            }
            ?>
            
            </datalist>
      </div>
      
      <div class="input-group">
      <?php if ($edit_state == false): ?>
        <button type="submit" name="save" class="btn">Sauvegarder</button>
      <?php else: ?>
        <button type="submit" name="update" class="btn">Mettre à jour</button>
      <?php endif ?>
      </div>
    </form>

<?php include 'footer.php'; ?>